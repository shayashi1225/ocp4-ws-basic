...
- A) 新規プロジェクトの作成 [(3-3-1)](#3-3-1-プロジェクト(namespace)の作成)
- B) カタログからデプロイ [(3-3-2)](#3-3-2-カタログでソースを指定してビルド&デプロイ)
- C) アプリのPod動作の確認 [(3-3-3)](#3-3-3-blogアプリケーションの状態を確認)
- D) Routerの作成 [(3-3-4)](#3-3-4-外部からアクセスするための-route-を作成)
- E) アプリの動作確認 [(3-3-5)](#3-3-5-アプリケーションの動作確認)
